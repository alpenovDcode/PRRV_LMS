      ...
      try {
        ...
        if (!funnelId || !fieldId) {
          return NextResponse.json(
            { error: "funnelId и fieldId обязательны" },
            { status: 400 }
          );
        }
        ...
        if (!bitrixUrl) {
          return NextResponse.json(
            { error: "Bitrix24 webhook URL не настроен" },
            { status: 500 }
          );
        }
        ...
        const parsed = submissions.map((sub) => {
          ...
          try {
            content = sub.content ? JSON.parse(sub.content) : {};
          } catch {
            content = {};
          }
        ...
        // --- STEP 3: Batch-search existing deals ---
        const dealIdMap: Record<string, string> = {};
        const withContact = parsed.filter(
          (p) => p.email && contactIdMap[p.email]
        );
        const noContact = parsed.filter(
          (p) => !p.email || !contactIdMap[p.email]
        );
        noContact.forEach((p) =>
          errors.push({
            ...
            cmds[`d${i}`] = {
              ...
              params: {
                filter: {
                  ...
                  CONTACT_ID: contactIdMap[p.email],
                },
                select: ["ID"],
                order: { ID: "DESC" },
              },
        ...
        for (const ch of updateChunks) {
          ...
          ch.forEach((p, i) => {
            if (results[`u${i}`]) {
              ...
              errors.push({
                id: p.subId,
                error: "Ошибка обновления сделки в Bitrix",
              });
            }
          });
        }
        ...
        if (errors.length > 0) {
          ...
          errors.forEach((e) => {
            byReason[e.error] = (byReason[e.error] ?? 0) + 1;
          });
          await logError({
