            ...
            {(() => {
              ...
              return groups.map((group, gIdx) => {
                ...
                if (group.type === 'full') {
                  ...
                  const design = { bg: "bg-white", textColor: "text-gray-900", textSize: "lg", padding: "py-24", container: "fixed", textAlign: "left" as const, accentColor: palette.accentColor, ...block.design };
                  return (
                    <motion.div key={block.id} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-10%" }} transition={{ duration: 0.8, delay: gIdx * 0.1, ease: "easeOut" }} className={`landing-block relative overflow-hidden transition-all duration-500 w-full ${layoutMode === 'cards' ? `${design.bg} rounded-[4.5rem] shadow-premium border border-gray-100` : ''}`}>
                        <BlockRenderer block={block} design={design} {...renderProps} />
                    </motion.div>
                  );
                } else {
                  return (
                    <div key={`grid-${gIdx}`} className="flex flex-col md:flex-row gap-8 items-start">
                      <div className="flex-1 flex flex-col gap-8 w-full">
                        {group.left.map((block: any, bIdx: number) => {
                          const design = { bg: "bg-white", textColor: "text-gray-900", textSize: "lg", padding: "py-24", container: "fixed", textAlign: "left" as const, accentColor: palette.accentColor, ...block.design };
                          return (
                            <motion.div key={block.id} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-10%" }} transition={{ duration: 0.8, delay: bIdx * 0.05, ease: "easeOut" }} className={`landing-block relative overflow-hidden transition-all duration-500 w-full ${layoutMode === 'cards' ? `${design.bg} rounded-[3.5rem] shadow-premium border border-gray-100` : ''}`}>
                              <BlockRenderer block={block} design={design} {...renderProps} />
                            </motion.div>
                          );
                        })}
                      ...
                      <div className="flex-1 flex flex-col gap-8 w-full">
                        {group.right.map((block: any, bIdx: number) => {
                          const design = { bg: "bg-white", textColor: "text-gray-900", textSize: "lg", padding: "py-24", container: "fixed", textAlign: "left" as const, accentColor: palette.accentColor, ...block.design };
                          return (
                            <motion.div key={block.id} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-10%" }} transition={{ duration: 0.8, delay: bIdx * 0.05, ease: "easeOut" }} className={`landing-block relative overflow-hidden transition-all duration-500 w-full ${layoutMode === 'cards' ? `${design.bg} rounded-[3.5rem] shadow-premium border border-gray-100` : ''}`}>
                              <BlockRenderer block={block} design={design} {...renderProps} />
